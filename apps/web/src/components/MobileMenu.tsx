        ...
        <div class="fixed inset-0 top-0 z-40 bg-base-100/95 backdrop-blur-xl lg:hidden transition-all duration-300 flex flex-col items-center justify-center p-6 pt-20">
          <div class="flex flex-col items-center gap-6 w-full max-w-sm">
            {navLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  class={`text-xl md:text-2xl font-serif transition-all duration-300 w-full text-center py-2 pond-shape ${isActive ? 'bg-sky-100 dark:bg-sky-900/40 text-primary dark:text-white font-bold shadow-sm' : 'text-base-content hover:bg-base-200 hover:text-primary font-medium'}`}
            ...
            <div class="w-full pt-8 mt-4 border-t border-base-content/10 flex flex-col gap-6">
              <a href="tel:+441234567890" class="flex items-center justify-center gap-2 text-base md:text-lg font-medium text-base-content/80 hover:text-primary transition-colors py-2">
